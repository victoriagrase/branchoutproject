...
The following equation is the core foundation of the algorithm. The dynamic programming formulation of optimal classification trees is given by Figure 1 where input parameters consist of binary data set ⅅ with features 𝐹, an upper bound on depth ⅆ, and an upper bound on the number of feature nodes 𝑛. The output is the minimum number of misclassifications possible on the data given the input decision tree characteristics. Here ∅ indicated that tree is infeasible or not of interest since a better tree is known.
